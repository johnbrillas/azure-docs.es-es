...
En la copia del repositorio de ejemplos de IoT de Microsoft Azure para C# que descargó anteriormente, abra el archivo del proyecto *azure-iot-samples-csharp-master\iot-hub\Samples\device\PnpDeviceSamples\Thermostat\Thermostat.csproj* en Visual Studio. En el proyecto **Thermostat**, abra los archivos *Program.cs* y *ThermostatSample.cs* para ver el código de este ejemplo.
...
A continuación, el método Main crea una instancia de **ThermostatSample** y llama al método `PerformOperationsAsync` para controlar las interacciones con IoT Central.

En *ThermostatSample.cs*, el método `PerformOperationsAsync`:

* Establece un controlador para recibir las actualizaciones de las propiedades deseadas de la temperatura objetivo.
* Establece un controlador para el comando **getMaxMinReport**.
* Envía datos de telemetría de temperatura periódicamente.
* Envía la temperatura máxima desde el último reinicio siempre que se alcance una nueva temperatura máxima.
...
El método `SendTemperatureAsync` muestra cómo el dispositivo envía la telemetría de temperatura a IoT Central:
...
El método `UpdateMaxTemperatureSinceLastRebootAsync` envía una actualización de la propiedad `maxTempSinceLastReboot` a IoT Central:
...
El método `TargetTemperatureUpdateCallbackAsync` controla la actualización de la propiedad de escritura de la temperatura objetivo desde IoT Central:
...
El método `HandleMaxMinReportCommand` controla el comando llamado desde IoT Central:
...
1. Abra el archivo del proyecto *azure-iot-samples-csharp-master/iot-hub/Samples/device/PnpDeviceSamples/Thermostat/Thermostat.csproj* en Visual Studio.
