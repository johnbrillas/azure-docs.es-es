...
Para corregir este problema, aplique una [AzurePodIdentityException](https://azure.github.io/aad-pod-identity/docs/configure/application_exception) para el *azds-injector-webhook* y actualice los pods instrumentados por Azure Dev Spaces para tener acceso a la identidad administrada.
...
El inicializador de Kubernetes no puede aplicar PodSpec a los nuevos pods debido a cambios en los permisos de RBAC de Kubernetes en el rol *cluster-admin* del clúster. El nuevo pod también puede tener un PodSpec no válido, por ejemplo si la cuenta de servicio asociada con el pod ya no existe. Para ver los pods que están en un estado *pendiente* debido a algún problema del inicializador, use el comando `kubectl get pods`:
...
### <a name="incorrect-azure-rbac-permissions-for-calling-dev-spaces-controller-and-apis"></a>Permisos incorrectos de RBAC de Azure para llamar al controlador y las API de Dev Spaces
