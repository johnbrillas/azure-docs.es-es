...
1. Cree un ámbito en la aplicación en la página **Exponer una API** de la aplicación y haga clic en "Agregar un ámbito".  El portal puede requerir también la creación de un identificador URI del identificador de aplicación. 
...
1. Elija **Seleccionar permisos** y, a continuación, seleccione el ámbito que creó en el último paso de registro del servicio de nivel intermedio.
...
El siguiente elemento HTTP POST solicita un token de acceso para la API web https://graph.microsoft.com. El parámetro `client_id` permite identificar el servicio que solicita el token de acceso.
...
El siguiente elemento HTTP POST solicita un token de acceso para la API web https://graph.microsoft.com con un certificado. El parámetro `client_id` permite identificar el servicio que solicita el token de acceso.
