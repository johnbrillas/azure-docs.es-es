---
title: 'Azure AD Connect: Archivo del historial de lanzamiento de versiones | Microsoft Docs'
description: En este artículo se muestran todas las versiones archivadas de Azure AD Connect y Sincronización de Azure AD.
...
>[!NOTE] 
> Este artículo contiene información de referencia sobre la versión de todas las versiones archivadas de Azure AD - 1.3.20.0 y anteriores.  Para las versiones actuales, consulte [Azure AD Connect: Historial de lanzamiento de versiones](reference-connect-version-history.md).
...
- La integración de PingFederate en Azure AD Connect ya tiene disponibilidad general. [Más información sobre cómo realizar la federación de Azure AD con PingFederate](./plan-connect-user-signin.md#federation-with-pingfederate)
- Ahora, Azure AD Connect crea la copia de seguridad de confianza de Azure AD en AD FS cada vez que se realiza una actualización, y la almacena en un archivo independiente para una sencilla restauración, si esta fuese necesaria. [Más información sobre la nueva funcionalidad y la administración de confianza de Azure AD en Azure AD Connect ](./how-to-connect-azure-ad-trust.md).
...
- Se ha corregido un error en el servidor de Azure AD Connect que mostraba un uso elevado de CPU después de actualizar a .NET 4.7.2.
...
-  Se ha agregado compatibilidad para el permiso que se concede para la reescritura de grupos en el asistente para la instalación de Azure AD Connect.
...
#### <a name="lock-down-access-to-the-ad-ds-account"></a><a name="lock"></a> Bloquear el acceso a la cuenta de AD DS
...
Para obtener más información, vea [Microsoft Security Advisory 4056318](/security-updates/securityadvisories/2017/4056318).
...
#### <a name="new-features-and-improvements"></a>Nuevas características y mejoras
* La tarea Comprobar inicio de sesión de ADFS de Azure AD Connect se ha actualizado de forma que ahora comprueba los inicios de sesión con Microsoft Online y no usa simplemente la recuperación de token de ADFS.
* Al configurar una nueva granja de servidores de ADFS mediante Azure AD Connect, se ha movido la página que solicitaba las credenciales de ADFS para que ahora aparezca antes de pedir al usuario que proporcione los servidores ADFS y WAP.  Esto permite que Azure AD Connect compruebe que la cuenta especificada tenga los permisos correctos.
* Durante la actualización de Azure AD Connect, ya no se producirá ningún error de actualización, aunque la confianza de Azure AD de ADFS provoque un error al actualizarse.  Si sucede eso, se le mostrará al usuario el mensaje de advertencia correspondiente, y este deberá continuar para restablecer la confianza mediante la tarea adicional de Azure AD Connect.
...
* Anteriormente, la característica [ms-DS-ConsistencyGuid como sourceAnchor](./plan-connect-design-concepts.md#using-ms-ds-consistencyguid-as-sourceanchor) estaba disponible solo para nuevas implementaciones. Ahora, está disponible para las implementaciones existentes. Más concretamente:
...
* En relación al atributo **userCertificate** en los objetos de dispositivo, Azure AD Connect ahora busca los valores de certificado necesarios para [Conectar dispositivos unidos a dominio a Azure AD para la experiencia de Windows 10](../devices/hybrid-azuread-join-plan.md) y filtra el resto antes de sincronizarse con Azure AD. Para permitir este comportamiento, la regla de sincronización inmediata "Out to AAD - Device Join SOAInAD" se ha actualizado.
...
* Cambios en las reglas de sincronización: se han implementado los siguientes cambios en reglas de sincronización:
  ...
  * Se ha agregado **preferredDataLocation** al esquema de Metaverse y al del conector de Azure AD. Los clientes que deseen actualizar cualquiera de los atributos en Azure AD pueden implementar reglas de sincronización personalizadas para hacerlo. 
  * Se ha agregado **userType** al esquema de Metaverse y al del conector de Azure AD. Los clientes que deseen actualizar cualquiera de los atributos en Azure AD pueden implementar reglas de sincronización personalizadas para hacerlo.
...
* Se ha corregido un problema que puede hacer que el servidor de Azure AD Connect no intente volver a conectarse si un firewall o un proxy termina la conexión.
...
* Admite varias organizaciones de Exchange locales. Consulte [Implementaciones híbridas con varios bosques de Active Directory](/previous-versions/exchange-server/exchange-150/jj873754(v=exchg.150)) para más información.
