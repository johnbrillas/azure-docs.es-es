---
title: 'Ejemplos de PowerShell y Graph para licencias de grupos: Azure AD | Microsoft Docs'
description: Ejemplos y escenarios de PowerShell y Graph para licencias basadas en grupos de Azure Active Directory
...
La funcionalidad completa de las licencias basadas en grupos está disponible a través de [Azure Portal](https://portal.azure.com) y, en la actualidad, hay algunas tareas útiles que se pueden realizar con los [cmdlets de PowerShell de MSOnline](/powershell/module/msonline) existentes y Microsoft Graph. Este documento proporciona ejemplos de lo que es posible.

> [!NOTE]
> Antes de comenzar a ejecutar los cmdlets, asegúrese de conectarse primero a su organización con el cmdlet `Connect-MsolService`.
...
## <a name="assign-licenses-to-a-group"></a>Asignación de licencias a un grupo

Use el ejemplo siguiente para asignar licencias a un grupo mediante Microsoft Graph:
...
El cmdlet [Get-MsolGroup](/powershell/module/msonline/get-msolgroup) se puede usar para recuperar el objeto de grupo y comprobar la propiedad *Licenses*: enumera todas las licencias de producto actualmente asignadas al grupo.
...
> [!NOTE]
> Los datos se limitan a la información de producto (SKU). No es posible mostrar los planes de servicio deshabilitados en la licencia.

Use el siguiente ejemplo para obtener los mismos datos de Microsoft Graph.
...
```
Use lo siguiente para obtener los mismos datos desde Microsoft Graph:
```
...
Use lo siguiente para obtener los mismos datos de Microsoft Graph:
...
## <a name="get-all-users-with-license-errors-in-the-entire-organization"></a>Obtención de todos los usuarios con errores de licencia en toda la organización

El script siguiente se puede usar para obtener todos los usuarios que tienen errores de licencia de uno o varios grupos. El script imprime una fila por cada usuario y cada error de licencia, lo que le permite identificar claramente el origen de cada error.

> [!NOTE]
> Este script enumera todos los usuarios de la organización que pudieran no ser óptimos para organizaciones grandes.
...
Graph no dispone de una manera sencilla de mostrar los resultados, pero se pueden ver desde esta API:
...
El propósito de este script es quitar las licencias directas no necesarias a los usuarios que heredan la misma licencia de un grupo; por ejemplo, como parte de una [transición a licencias basadas en grupos](licensing-groups-migrate-users.md).
> [!NOTE]
...
```
> [!NOTE]
> Actualice los valores de las variables `$skuId` y `$groupId` que se van a destinar para la eliminación de licencias directas según el entorno de prueba antes de ejecutar el script anterior. 
