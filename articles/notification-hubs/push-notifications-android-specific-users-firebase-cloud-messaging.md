...
description: Aprenda a enviar notificaciones push a aplicaciones Android concretas mediante Azure Notification Hubs.
...
12. En el archivo `build.gradle` agregue la siguiente línea a la sección `android` después de la sección `buildTypes`.
    ...
    ```
13. Si su aplicación tiene como destino el nivel 28 de API (Android 9.0) o posterior, incluya la siguiente declaración `<application>` dentro del elemento `AndroidManifest.xml`.
...
3. En la aplicación de Android, haga clic en **Iniciar sesión**. Espere que aparezca un mensaje de aviso que indique que **se ha iniciado la sesión y se ha registrado**. Así se habilita el botón **Enviar notificación**.
    ...
    ![Captura de pantalla de un emulador que muestra el aspecto de la aplicación para notificar a los usuarios de Notification Hubs después de iniciar sesión.][A2]
