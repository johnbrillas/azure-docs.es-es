...
Introducción a las etiquetas de servicio de Azure: https://docs.microsoft.com/azure/virtual-network/service-tags-overview
...
**Guía**: No aplicable; Azure Key Vault no tiene el concepto de contraseñas predeterminadas, ya que la autenticación la proporciona Active Directory y se protege con el control de acceso basado en roles de Azure (Azure RBAC).
...
**Instrucciones**: Todo el tráfico dirigido a Azure Key Vault para la autenticación, la administración y el acceso al plano de datos se cifra y pasa a través de HTTPS: puerto 443. Sin embargo, ocasionalmente habrá tráfico HTTP (puerto 80) para CRL. Azure Key Vault continúa permitiendo la ingesta de datos de TLS 1.1 y TLS 1.0. Los datos se pueden restringir a TLS 1.2 mediante la configuración en el lado cliente.
