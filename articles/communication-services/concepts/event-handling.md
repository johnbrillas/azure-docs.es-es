...
| Microsoft.Communication.ChatParticipantAddedToThreadWithUser|  Se publica para un usuario cuando se agrega un participante a un subproceso de chat del que forma parte el usuario.|
| Microsoft.Communication.ChatParticipantRemovedFromThreadWithUser |  Se publica para un usuario cuando se quita un participante de un subproceso de chat del que forma parte el usuario. |
| Microsoft.Communication.ChatThreadCreated  | Se publica cuando se crea un subproceso de chat.  |
| Microsoft.Communication.ChatThreadDeleted| Se publica cuando se elimina un subproceso de chat.  |
| Microsoft.Communication.ChatThreadParticipantAdded | Se publica cuando se agrega un participante a un subproceso de chat.  |
| Microsoft.Communication.ChatThreadParticipantRemoved | Se publica cuando se agrega un participante a un subproceso de chat.  |  
| Microsoft.Communication.ChatMessageReceivedInThread | Publicado cuando se recibe un mensaje en un subproceso de chat.  |    
...
El campo `subject` de todos los eventos de Communication Services identifica el usuario, el número de teléfono o la entidad de destino del evento. Los prefijos comunes se utilizan para permitir el [filtrado de Event Grid](../../event-grid/event-filtering.md) simple.
...
* Para obtener una introducción a Azure Event Grid, consulte [Conceptos de Event Grid](../../event-grid/concepts.md).
* Para obtener una introducción a Azure Event Grid, consulte [Temas del sistema en Azure Event Grid](../../event-grid/system-topics.md).
