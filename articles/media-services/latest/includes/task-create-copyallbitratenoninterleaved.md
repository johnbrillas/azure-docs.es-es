...
ms.locfileid: "96509229"
---
<!--Create a copyAllNonInterleave transform with REST-->

El siguiente comando de REST de Azure crea una transformación mediante `copyAllBitrateNonInterleaved` del valor preestablecido `#Microsoft.Media.BuiltInStandardEncoderPreset`. Reemplace los valores `subscriptionID`, `resourceGroup` y `accountName` por los valores con los que esté trabajando actualmente. Asigne un nombre a la transformación; para ello, establezca `transformName`. 

Para ver todas las transformaciones de API REST disponibles, consulte [Transformaciones - Crear o actualizar](/rest/api/media/transforms/createorupdate#definitions).
