...
ms.locfileid: "104576905"
---
# <a name="azure-security-baseline-for-azure-iot-hub"></a>Línea de base de seguridad de Azure para Azure IoT Hub

Esta base de referencia de seguridad aplica la guía de la [versión 1.0 de Azure Security Benchmark](../security/benchmarks/overview-v1.md) en Microsoft Azure IoT Hub. Azure Security Benchmark proporciona recomendaciones sobre cómo puede proteger sus soluciones de nube en Azure.
El contenido se agrupa por medio de los **controles de seguridad** que define Azure Security Benchmark y las guías correspondientes aplicables a Azure IoT Hub. Se han excluido los **controles** no aplicables a Azure IoT Hub.

 
Para ver cómo Azure IoT Hub se adapta por completo a Azure Security Benchmark, consulte el [archivo completo de asignación de base de referencia de seguridad de Azure IoT Hub](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Guía**: IoT Hub es una plataforma como servicio (PaaS) multiinquilino, por lo que diferentes clientes comparten el mismo grupo de recursos de proceso, red y hardware de almacenamiento. Los nombres de host de IoT Hub se asignan a un punto de conexión público con una dirección IP enrutable públicamente a través de Internet. Diferentes clientes comparten este punto de conexión público de IoT Hub, que es accesible para todos los dispositivos de IoT a través de redes de área extensa y de redes locales. Microsoft diseñó el servicio para un aislamiento completo entre los datos de cada inquilino y sigue trabajando continuamente para garantizar este resultado.
...
Una vez establecido el acceso privado, deshabilite el acceso a la red pública para la instancia de IoT Hub para mayor seguridad. Este control de nivel de red se aplica sobre un recurso de IoT Hub específico, lo que garantiza el aislamiento. Para mantener el servicio activo para otros recursos del cliente usando la ruta de acceso pública, su punto de conexión público aún se puede resolver, las direcciones IP aún se pueden detectar y los puertos permanecen abiertos. Esto no es motivo de preocupación, ya que Microsoft integra varias capas de seguridad para garantizar el aislamiento completo entre los inquilinos.
...
- [Aislamiento en el nivel de inquilino](https://docs.microsoft.com/azure/security/fundamentals/isolation-choices#tenant-level-isolation)

- [Procedimientos recomendados de redes de IoT](https://docs.microsoft.com/azure/iot-fundamentals/security-recommendations#networking)
...
- [Azure Security Center para la comunicación de direcciones IP sospechosas de IoT](/azure/asc-for-iot/concept-security-alerts)
...
- [Configuración de registros de Azure IoT](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub-reference#resource-logs)
...
- [Configuración de registros de Azure IoT Hub](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub-reference#resource-logs)
...
**Definiciones integradas de Azure Policy: Microsoft.Devices**:
...
- [Alertas de Azure Security Center para IoT](/azure/asc-for-iot/concept-security-alerts)
...
También puede habilitar el acceso Just-in-Time a cuentas administrativas mediante Azure Active Directory (Azure AD) Privileged Identity Management y Azure Resource Manager.
...
**Guía**: los usuarios que acceden a IoT Hub, deben usar el inicio de sesión único de Azure Active Directory (Azure AD). Use las recomendaciones de identidades y acceso de Azure Security Center.
...
**Guía**: habilite la autenticación multifactor de Azure Active Directory (Azure AD) para proteger el inquilino general de Azure y beneficiarse así a todos los servicios. El servicio IoT Hub no es compatible con la autenticación multifactor.
...
**Guía**: use una estación de trabajo de acceso con privilegios (PAW) segura para las tareas administrativas que requieren privilegios elevados.

- [Descripción de las estaciones de trabajo seguras con privilegios de acceso](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/)
...
- [Tokens de seguridad de IoT Hub](https://docs.microsoft.com/azure/iot-fundamentals/iot-security-deployment#iot-hub-security-tokens)
...
- [Configuración de registros de recursos para IoT Hub](https://docs.microsoft.com/azure/iot-hub/monitor-iot-hub#collection-and-routing)
...
- [Uso de identidades administradas para IoT Hub](https://docs.microsoft.com/azure/iot-hub/virtual-network-support#turn-on-managed-identity-for-iot-hub)
...
- [Configuración de las identidades administradas para IoT Hub](https://docs.microsoft.com/azure/iot-hub/virtual-network-support#turn-on-managed-identity-for-iot-hub)
...
- [Alta disponibilidad y recuperación ante desastres de IoT Hub](https://docs.microsoft.com/azure/iot-hub/iot-hub-ha-dr#achieve-cross-region-ha)
