...
### <a name="managing-concurrent-connections-version-2x"></a><a name="jobhost-servicepointmanager-settings"></a>Administración de conexiones simultáneas (versión 2.*x*)
...
Para más información, consulte el artículo [Enlaces de Azure CosmosDB](../azure-functions/functions-bindings-cosmosdb-v2-output.md#hostjson-settings).
...
Para más información, consulte el artículo [Enlace de Event Hubs](../azure-functions/functions-bindings-event-hubs.md#host-json).
...
El objeto `SamplingPercentageEstimatorSettings` configura el [muestreo adaptable](../azure-monitor/app/sampling.md). Esto significa que, en ciertos escenarios de gran volumen, App Insights envía un subconjunto de datos de telemetría seleccionados al servidor.
