...
`{Log Level}` debe seguir el [formato de nivel de gravedad de Syslog](https://wikipedia.org/wiki/Syslog#Severity_level) y `{Timestamp}` debe tener el formato `yyyy-MM-dd hh:mm:ss.fff zzz`.
