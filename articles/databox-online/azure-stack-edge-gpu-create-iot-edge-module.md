---
title: Módulo de IoT Edge en C# para Azure Stack Edge Pro con GPU | Microsoft Docs
description: Aprenda a desarrollar un módulo de IoT Edge en C# que pueda implementarse en un dispositivo Azure Stack Edge Pro con GPU.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
![Funcionamiento del módulo de Azure IoT Edge en Azure Stack Edge Pro](./media/azure-stack-edge-gpu-create-iot-edge-module/how-module-works-1.png)
...
- Un dispositivo Azure Stack Edge Pro en ejecución.
    ...
    Para más información, vaya a [Configuración del proceso](azure-stack-edge-j-series-deploy-configure-compute.md#configure-compute) en Azure Stack Edge Pro.<!--Update link?-->
...
3. Proporcione:
   ...
   4. Seleccione una **ubicación**. Es conveniente que esta ubicación sea la misma que está asociada con el recurso de Azure Stack Edge.
   5. Establezca **Usuario administrador** en **Habilitar**.
   6. Establezca la SKU en **Básica**.

      ![Crear un registro de contenedor](./media/azure-stack-edge-gpu-create-iot-edge-module/create-container-registry-1.png)
 
4. Seleccione **Crear**.
5. Una vez que se haya creado el Registro de contenedor, desplácese hasta él y seleccione **Claves de acceso**.

    ![Obtención de las claves de acceso](./media/azure-stack-edge-gpu-create-iot-edge-module/get-access-keys-1.png)
...
3. En la paleta de comandos, escriba y ejecute el comando **Azure IoT Edge: New IoT Edge solution** (Azure IoT Edge: nueva solución de IoT Edge). En la paleta de comandos, proporcione la siguiente información para crear la solución:
    ...
    2. Proporcione un nombre para la solución o acepte el valor predeterminado: **EdgeSolution**.
        ...
        ![Creación de la solución 1](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-1.png)
    ...
    4. Reemplace el nombre de módulo predeterminado por el nombre que quiere asignar, en este caso, **FileCopyModule**.
        ...
        ![Creación de la solución 2](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. Especifique el registro de contenedor que creó en la sección anterior como repositorio de imágenes del primer módulo. Reemplace **localhost:5000** por el valor del servidor de inicio de sesión que copió.
        ...
        ![Creación de la solución 3](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-3.png)

4. Vaya a **File > Open Folder** (Archivo > Abrir carpeta).

    ![Creación de la solución 4](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-4.png)

5. Examine y seleccione la carpeta **EdgeSolution** que creó anteriormente. La ventana de VS Code carga el área de trabajo de la solución de IoT Edge con sus cinco componentes de nivel superior. En este artículo, no editará la carpeta `.vscode` ni los archivos **.gitignore**, **.env** y deployment.template.json**.
    ...
    ![Creación de la solución 5](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-5.png)
...
2. Escriba el comando siguiente en el terminal integrado para iniciar sesión en Docker.
    ...
    ![Compilación e inserción de la solución de IoT Edge](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. Una vez que se proporcionan las credenciales, puede insertar la imagen del módulo en su registro de contenedor de Azure. En el explorador de VS Code, haga clic con el botón derecho en el archivo **module.json** y seleccione **Build and Push IoT Edge solution** (Compilar e insertar la solución de IoT Edge).
    ...
    ![Compilar e insertar solución IoT Edge 2](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    ![Seleccionar plataforma](./media/azure-stack-edge-gpu-create-iot-edge-module/select-platform.png)
...
Para implementar y ejecutar este módulo en Azure Stack Edge Pro, consulte los pasos que se indican en [Adición de un módulo](azure-stack-edge-j-series-deploy-configure-compute.md#add-a-module).<!--Update link?-->
